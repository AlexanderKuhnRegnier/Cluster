...
//	printfprintf "/cluster/data/raw/".$year."/".$month."/".$base.".SATT";

//	debug("WrEB ".count($outputdata));
...
$new[S_IDLE][DBLE]=	array(	P_INIT=>	S_IDLE,	P_BAD=>			S_IDLE,	P_ERR=>			S_IDLE,
							P_ODD=>		S_ODD,	P_ODD_BADEND=>	S_IDLE,	P_ODD_BADSRT=>	S_ODD,
							P_EVEN=>	S_EVEN,	P_EVEN_BADEND=>	S_IDLE,	P_EVEN_BADSRT=>	S_EVEN);
...
// becase it *WILL* be wrong as well (doh!)
		     ...
		     (($previousreset-$telemreset)==65535) )
		{
			if ( ($previousextreset==999999) ||			// ... and this is the first extended mode or extended mode
			     ...
			     (($s-$previousextreset)==1)) ||
			     (($previousextreset-$e)==4095) )
			{
