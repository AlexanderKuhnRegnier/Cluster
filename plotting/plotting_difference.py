...
class vectorlist:
    ...
    def plotlists(self,array,scatter=False,log=True):
        # output to static HTML file (with CDN resources)
        output_file("plotdifference.html", title="Plotting Cluster II Data, DEFAULT CAL", mode="cdn")
        ...
        for entry in array:
            ...
            if 'diff' not in plotwhich:
                dates = vlist.returndatetimes()
            else:
                dates = vlist[0].returndatetimes()
            if plotwhich=='mag':
                ...
                data=vlist.returnz()
            elif plotwhich=='diffmag':
                vlist1=vlist[0]
                vlist2=vlist[1]
                data1=vlist1.returnmagnitudes()
                data2=vlist2.returnmagnitudes()
                data = [data1[i]-data2[i] for i in range(len(data1))]
            elif plotwhich=='diffx':
                vlist1=vlist[0]
                vlist2=vlist[1]
                data1=vlist1.returnx()
                data2=vlist2.returnx()
                data = [data1[i]-data2[i] for i in range(len(data1))]
            elif plotwhich=='diffz':
                vlist1=vlist[0]
                vlist2=vlist[1]
                data1=vlist1.returnz()
                data2=vlist2.returnz()
                data = [data1[i]-data2[i] for i in range(len(data1))]
            else:
                data=vlist.returnmagnitudes()
            print "data"
...
filename1 = "Y:/reference/2015/12/DEFAULTC1_151231_B.EXT.GSE"
filename2 = "Y:/reference/2015/12/CAAC1_151231_B.EXT.GSE"

vlist1 = vectorlist()
vlist1.read_file(filename1)
#vlist1.print_values(30)
vlist2 = vectorlist()
vlist2.read_file(filename2)

vlist = vectorlist()
vlist.plotlists([[(vlist1,vlist2),'purple','diffz','diffz'],[(vlist1,vlist2),'blue','diffmag','diffmag']],log=False)
